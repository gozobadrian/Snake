...
string encodeDecode(string toEncrypt) // Encrypts/Decrypts highscore file
{
...
void addPlayer() // Adds player to map
{
    for (int i = 1; i <= player1.lenght; i++)
    {
        if (i == 1)
        {
            map1.addPlayerHeadToMap(player1.chain[i].playerX, player1.chain[i].playerY);
        }
...
void loadHighScore() // Reads highscore file
{
    ...
    player1.name = encodeDecode(encryptedString);
...
void newHighscore() // Prints new highscore message and
{
    ...
    {
        scoreOut << encodeDecode(player1.name) << '\n';
    }
